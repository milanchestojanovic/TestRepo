...
///// milan ////
	 ...
	 for( unsigned ic = 0; ic < pfCandidates->size(); ic++ ){
	      ...
	      double pt = cand.p4().Pt(), ptref = tmtr->pt();
//
              if (pt == ptref )  std::cout <<" radi "<<std::endl;
//     

	      reco::TrackRef trackRef = cand.trackRef();
	      if( index == trackRef.key() ) {
		if (tmtr->pt() != cand.p4().Pt() && tmtr->phi()!=cand.p4().Phi() && tmtr->eta()!=cand.p4().Eta())	
//  
//
		signal->Fill(tmtr->pt()-cand.p4().Pt(), tmtr->phi()-cand.p4().Phi(),tmtr->eta()-cand.p4().Eta());
	        ecalEnergy = cand.ecalEnergy();
	        hcalEnergy = cand.hcalEnergy();
      
	        break;
	      }
	 if (tmtr->pt() == cand.p4().Pt() && tmtr->phi()==cand.p4().Phi() && tmtr->eta()==cand.p4().Eta()    )	std::cout <<" radi "<<std::endl;

	

	 }

	if (tp->pt() > 20) std::cout<<"first condition : "<<( (hcalEnergy+ecalEnergy)/( tp->pt()*TMath::CosH(tp->eta() ) ) )<<"  second condition : "<<(hcalEnergy+ecalEnergy)/(TMath::CosH(tp->eta()))<<std::endl;
